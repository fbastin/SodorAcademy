...
import { Train, Trophy, Star, Shield, Layout, BookOpen, Calculator, RotateCcw } from 'lucide-react';
import { Grade, Subject, Question, UserStats, SUBJECTS, ENGINES } from './types';
import { generateQuestion } from './services/geminiService';

// --- Components ---

const Navbar = ({ stats }: { stats: UserStats }) => (
  <nav className="h-16 border-b bg-white flex items-center justify-between px-6 sticky top-0 z-50">
    <div className="flex items-center gap-2">
      ...
      <span className="font-extrabold text-xl tracking-tight text-sodor-blue uppercase">Sodor Academy</span>
    </div>
    <div className="flex items-center gap-4">
      ...
      <div className="flex items-center gap-2 bg-sodor-blue/10 px-3 py-1.5 rounded-full border border-sodor-blue/20">
        ...
        <span className="font-bold text-sodor-blue">{stats.enginesCollected.length} Engines</span>
      </div>
    </div>
  </nav>
);
...
export default function App() {
  ...
  const handleLessonComplete = (rewardType: string) => {
    const randomEngine = ENGINES[Math.floor(Math.random() * ENGINES.length)];
    setStats(prev => ({
      ...prev,
      score: prev.score + 100,
      completedLessons: prev.completedLessons + 1,
      enginesCollected: Array.from(new Set([...prev.enginesCollected, randomEngine.id]))
    }));
          ...
          {!activeSubject ? (
            ...
            >
              <div className="text-center mb-16">
                ...
                >
                  <button 
                    onClick={() => setStats(s => ({ ...s, currentGrade: 'Primary' }))}
                    className={`px-6 py-2 rounded-xl font-bold transition-all ${stats.currentGrade === 'Primary' ? 'bg-white text-sodor-blue shadow-sm' : 'text-slate-500'}`}
                  >
                    Primary School
                  </button>
                  <button 
                    onClick={() => setStats(s => ({ ...s, currentGrade: 'Secondary' }))}
                    className={`px-6 py-2 rounded-xl font-bold transition-all ${stats.currentGrade === 'Secondary' ? 'bg-white text-sodor-blue shadow-sm' : 'text-slate-500'}`}
                  >
                    Secondary Academy
                  </button>
                </motion.div>
                ...
                <h1 className="text-5xl font-black text-slate-900 mb-4 tracking-tight">
                  Welcome to Sodor <span className="text-sodor-blue">Academy</span>
                </h1>
              ...
              <div className="mt-24">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  {ENGINES.map(engine => {
                    const isCollected = stats.enginesCollected.includes(engine.id);
                    return (
                      <div 
                        key={engine.id}
